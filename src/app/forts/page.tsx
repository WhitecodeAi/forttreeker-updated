...
// lucide-react export names can vary. 'SlidersHorizontal' is standard.
...
export default function Forts() {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                ...
                const uniqueApprovedForts = (approvedFortsResponse.data || []).reduce((acc: any[], fortInfo: any) => {
                    ...
                    const existingFort = acc.find((f: any) => f.fort_name === fortInfo.fort_name);
                    if (!existingFort) {
                        acc.push(fortInfo);
                    }
                    return acc;
        ...
        // Note: 'fortDeleted' event might need custom dispatching if it relied on React Router/App structure.
        // Assuming it's a window event string.
    ...
    const filteredForts = useMemo(() => {
        ...
        filtered.sort((a, b) => {
            switch (sortBy) {
                ...
                case "difficulty":
                    const difficultyOrder: { [key: string]: number } = { "Easy": 1, "Moderate": 2, "Difficult": 3, "Expert": 4 };
